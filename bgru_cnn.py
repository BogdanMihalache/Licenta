...
def build_model(input_shape):
    ...
    # BiGRU-RNN block
    pool21 = keras.layers.MaxPool2D((2,2), strides=(2,2))(inputs)

    # Remove channel axis so we can pass the data into Bidirectional GRU layer
    squeezed = keras.layers.Lambda(lambda x: keras.backend.squeeze(x, axis=-1))(pool21)

    # Bidirectional GRU
    bigru_rnn = keras.layers.Bidirectional(keras.layers.GRU(128), merge_mode='concat')(squeezed)

    # Concat Output
    concat = keras.layers.concatenate([flatten, bigru_rnn])
